...
def grounding_eval(results,dets,cocos,refer,alpha,filename,mask_size=24):
    
    correct_A_d, correct_B_d, correct_val_d = 0, 0, 0
    correct_A, correct_B, correct_val = 0, 0, 0 
    num_A,num_B,num_val = 0,0,0
    successid = []
    for sampleid, res in tqdm(enumerate(results)):
    ...
    print(successid)   
    np.save(f'{filename}/si.npy',np.array(successid))
